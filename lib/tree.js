...
proto.moveIndex = function (index, dragInfo, e, canMoveFn, canMoveToCollapaed = false) {
  var newIndex = null;
  ...
  if (dragEl && plhEl) {
    ...
    if (!plhRect.width) {
        //todo: Bug #3: On collapsed node
        console.log('on collapsed node');
        return;
    }
        ...
        if (trgRect) {
          ...
          availMoves = availMoves.filter(am => {
            ...
            if ((placement == 'before' || placement == 'after') && trg.id == 1)
              return false;
            if (!canMoveToCollapaed && trg.node.collapsed && (placement == 'append' || placement == 'prepend'))
              return false;
